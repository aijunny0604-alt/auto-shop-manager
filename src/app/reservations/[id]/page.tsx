        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="scheduledAt" type="datetime-local" defaultValue={reservation.scheduledAt.slice(0, 16)} className="glass-input w-full rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">소요 시간 (분)</label>
              <input name="duration" type="number" defaultValue={reservation.duration} className="glass-input w-full rounded-lg px-3 py-2 text-sm" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1">작업 유형</label>
              <select name="serviceType" defaultValue={reservation.serviceType} className="glass-input w-full rounded-lg px-3 py-2 text-sm">
                {SERVICE_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">상태</label>
              <select name="status" defaultValue={reservation.status} className="glass-input w-full rounded-lg px-3 py-2 text-sm">
          ...
          <div>
            ...
            <textarea name="description" rows={3} defaultValue={reservation.description || ""} className="glass-input w-full rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">메모</label>
            <textarea name="memo" rows={2} defaultValue={reservation.memo || ""} className="glass-input w-full rounded-lg px-3 py-2 text-sm" />
          </div>
