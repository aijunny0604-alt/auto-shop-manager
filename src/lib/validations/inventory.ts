...
export const createInventorySchema = z.object({
  ...
  quantity: z.number().int().min(0, "수량은 0 이상이어야 합니다.").optional().default(0),
  minQuantity: z.number().int().min(0, "최소 수량은 0 이상이어야 합니다.").optional().default(5),
  unitPrice: z.number().int().min(0, "단가는 0 이상이어야 합니다.").optional().default(0),
